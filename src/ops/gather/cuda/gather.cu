...
__global__ void gather(
    ...
    Idata index = indices[idx / *stride_axis % *num_indices];
...
template<typename Tdata, typename Idata>
infiniopStatus_t gather_nv_gpu(GatherCudaDescriptor_t desc, void *dst, void const *data, void const *indices, void *stream) {
    if (desc->dst_size == 0) {
        return STATUS_SUCCESS;
    }
    dim3 blockDims = dim3(std::min(static_cast<uint64_t>(256), desc->dst_size));
    dim3 gridDims = dim3(std::min(ROUND_UP_DIV(desc->dst_size, blockDims.x), desc->max_grid_size));
    ...
    const uint64_t *num_indices = reinterpret_cast<const uint64_t *>(desc->num_indices_ptr);
    
    cudaStream_t cuda_stream = reinterpret_cast<cudaStream_t>(stream);

#pragma unroll
    for (uint64_t i = 0; i < desc->dst_size; i += step) {
