...
def reduceMean(x, axis = 0, keepdims = True):
...
def test(
    ...
    print(
        f"Testing ReduceMean on {torch_device} with tensor_shape:{tensor_shape} dtype:{tensor_dtype} axis:{axis} keepdim:{keepdims} inplace: {inplace.name}"
    )  
    ...
    for i in range(NUM_PRERUN if PROFILE else 1):
        ans = reduceMean(x, axis, bool(keepdims))

    if PROFILE:
        start_time = time.time()
        for i in range(NUM_ITERATIONS):
            _ = reduceMean(x, axis, bool(keepdims))
    ...
    if PROFILE:
        ...
        for i in range(NUM_ITERATIONS):
            check_error(
                lib.infiniopReduceMean(descriptor, workspace_ptr, workspaceSize, y_tensor.data, x_tensor.data, None)
            )
        elapsed = (time.time() - start_time) / NUM_ITERATIONS
...
if __name__ == "__main__":
    test_cases = [
        ...
        ((2, 3, 4, 5), [1, 2], 1, Inplace.OUT_OF_PLACE),
